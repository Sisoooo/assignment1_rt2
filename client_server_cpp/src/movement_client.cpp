...
namespace client_server_cpp{
  class MovementClient : public rclcpp::Node{
    ...
    public:
      ...
      MovementClient(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
      : Node("movement_client", options), cancel_sent_(false)
      {
    ...
    private:
      ...
      void goal_input_callback(const geometry_msgs::msg::Pose2D::SharedPtr msg)
      {
        ...
        broadcast_goal_frame(desired_x_, desired_y_, desired_theta_);

        if (!action_client_->action_server_is_ready()) {
          RCLCPP_WARN(this->get_logger(), "Action server not yet available, goal dropped");
          return;
        }

        auto goal_msg = Movement::Goal();
        goal_msg.desired_x = desired_x_;
        goal_msg.desired_y = desired_y_;
        goal_msg.desired_theta = desired_theta_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(client_server_cpp::MovementClient)
